...
protocol OtherMessageCollectionViewReplyStickerCellDelegate: AnyObject {
...
final class OtherMessageCollectionViewReplyStickerCell: UICollectionViewCell, UIGestureRecognizerDelegate {
    
    static let nib = UINib(nibName: "OtherMessageCollectionViewReplyStickerCell", bundle: nil)

    @IBOutlet weak var partnerUserIconView: UIImageView!
    ...
    weak private var delegate: OtherMessageCollectionViewReplyStickerCellDelegate?
    ...
    private func setUpTapGesture() {
        ...
        partnerUserIconView.isUserInteractionEnabled = true
        
        let iconImageViewTapGesture = UITapGestureRecognizer(
            target: self,
            action: #selector(onIconImageViewTapped(_:))
        )
        partnerUserIconView.addGestureRecognizer(iconImageViewTapGesture)
    ...
    private func setupViews() {
        ...
        partnerUserIconView.clipsToBounds = true
        partnerUserIconView.rounded()
        partnerUserIconView.contentMode = .scaleAspectFill
    }
    ...
    @objc private func onIconImageViewTapped(_ sender: UITapGestureRecognizer) {
        if let user { delegate?.onProfileIconTapped(cell: self, user: user) }
    }
    ...
    func configure(_ user: User, message: Message, room: Room?, delegate: OtherMessageCollectionViewReplyStickerCellDelegate, indexPath: IndexPath) {
        ...
        partnerUserIconView.setImage(withURLString: user.profile_icon_img)
